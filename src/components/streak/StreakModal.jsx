...
export default function StreakModal({ isOpen, onClose, streakState, freezeState, title, variant = 'neutral' }) {
  const { t } = useLanguage();
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="reaction-modal" titleId={title ? 'streak-modal-title' : undefined}>
      ...
      <div className="reaction-modal__dialog" data-variant={variant}>
        ...
        <StreakCalendar streakState={streakState} freezeState={freezeState} />
      </div>
    </Modal>
  );
}
