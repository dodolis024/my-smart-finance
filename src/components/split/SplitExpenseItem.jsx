export default function SplitExpenseItem({ expense, members, onDelete }) {
  const memberMap = Object.fromEntries((members || []).map(m => [m.id, m.name]));
  const paidByName = expense.paid_by ? memberMap[expense.paid_by] || '—' : '—';

  return (
    <div className="split-expense-item">
      ...
      <button type="button" className="split-expense-item__delete" onClick={() => onDelete(expense.id)} aria-label="刪除費用">
