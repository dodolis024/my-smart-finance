-- =============================================================================
-- ⛔ 已停用（2026-08-29）——請勿執行，改用 docs/EXCHANGE_RATE_SETUP_GUIDE.md 步驟 6
-- =============================================================================
-- 保留此檔僅供歷史對照（docs/DEPLOYMENT.md 的執行紀錄有引用檔名）。
--
-- 不可執行的兩個理由：
-- 1. 含 <YOUR_SUPABASE_URL> placeholder。此檔曾被原封貼上執行，command 留著
--    placeholder，pg_cron 每天照跑但請求從未離開資料庫，匯率靜默停擺 53 天才被發現。
-- 2. 缺少 x-cron-secret header。2026-08-27 起 Edge Function 強制驗證共用密鑰，
--    即使正確替換了 placeholder，建出來的排程也會被 401 擋掉——而 cron 仍顯示 succeeded。
--
-- 正確做法在 docs/EXCHANGE_RATE_SETUP_GUIDE.md：URL 已寫死、含密鑰與逾時設定、
-- 附 placeholder 防呆 DO block。
-- =============================================================================

-- =============================================================================
-- 自動更新匯率設定 SQL（以下為歷史內容）
-- =============================================================================
...
-- 注意：Supabase 可能需要先啟用此擴展，請在 Dashboard > Database > Extensions 中啟用
